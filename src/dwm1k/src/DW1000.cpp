...
void DW1000Class::begin(uint8_t irq, uint8_t rst) {
    ...
    gpioUnexport(pinRST_);
    usleep(tsGPIOWait_);
    gpioExport(pinRST_);
    usleep(tsGPIOWait_);

    gpioUnexport(pinIRQ_);
    usleep(tsGPIOWait_);
    gpioExport(pinIRQ_);
    usleep(tsGPIOWait_);
...
void DW1000Class::manageLDE() {
    ...
    usleep(TSMS_WAIT); readBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0);
    usleep(TSMS_WAIT); readBytes(OTP_IF, OTP_CTRL_SUB, otpctrl, LEN_OTP_CTRL);
    pmscctrl0[0] = 0x01;
    pmscctrl0[1] = 0x03;
    otpctrl[0]   = 0x00;
    otpctrl[1]   = 0x80;
    usleep(TSMS_WAIT); writeBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0);
    usleep(TSMS_WAIT); writeBytes(OTP_IF, OTP_CTRL_SUB, otpctrl, LEN_OTP_CTRL);
    usleep(5000);
    pmscctrl0[0] = 0x00;
    pmscctrl0[1] &= 0x02;
    usleep(TSMS_WAIT); writeBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0);
...
void DW1000Class::enableClock(byte clock) {
    ...
    usleep(TSMS_WAIT); writeBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, 2);
}
...
void DW1000Class::reset() {
    ...
    if(val == 1) {
        ...
        // dw1000 data sheet v2.08 §5.6.1 page 20, the RSTn pin should not be driven high but left floating.
        gpioSetDirection(pinRST_, outputPin);
        usleep(tsGPIOWait_);
        gpioSetValue(pinRST_, 0);
        usleep(tsGPIOWait_);
        // usleep(2000);  // dw1000 data sheet v2.08 §5.6.1 page 20: nominal 50ns, to be safe take more time
        gpioSetDirection(pinRST_, inputPin);
        usleep(tsGPIOWait_);
        // usleep(10000); // dwm1000 data sheet v1.2 page 5: nominal 3 ms, to be safe take more time
...
void DW1000Class::tune() {
    ...
    usleep(TSMS_WAIT); writeBytes(AGC_TUNE, AGC_TUNE1_SUB, agctune1, LEN_AGC_TUNE1);
    usleep(TSMS_WAIT); writeBytes(AGC_TUNE, AGC_TUNE2_SUB, agctune2, LEN_AGC_TUNE2);
    usleep(TSMS_WAIT); writeBytes(AGC_TUNE, AGC_TUNE3_SUB, agctune3, LEN_AGC_TUNE3);
    usleep(TSMS_WAIT); writeBytes(DRX_TUNE, DRX_TUNE0b_SUB, drxtune0b, LEN_DRX_TUNE0b);
    usleep(TSMS_WAIT); writeBytes(DRX_TUNE, DRX_TUNE1a_SUB, drxtune1a, LEN_DRX_TUNE1a);
    usleep(TSMS_WAIT); writeBytes(DRX_TUNE, DRX_TUNE1b_SUB, drxtune1b, LEN_DRX_TUNE1b);
    usleep(TSMS_WAIT); writeBytes(DRX_TUNE, DRX_TUNE2_SUB, drxtune2, LEN_DRX_TUNE2);
    usleep(TSMS_WAIT); writeBytes(DRX_TUNE, DRX_TUNE4H_SUB, drxtune4H, LEN_DRX_TUNE4H);
    usleep(TSMS_WAIT); writeBytes(LDE_IF, LDE_CFG1_SUB, ldecfg1, LEN_LDE_CFG1);
    usleep(TSMS_WAIT); writeBytes(LDE_IF, LDE_CFG2_SUB, ldecfg2, LEN_LDE_CFG2);
    usleep(TSMS_WAIT); writeBytes(LDE_IF, LDE_REPC_SUB, lderepc, LEN_LDE_REPC);
    usleep(TSMS_WAIT); writeBytes(TX_POWER, NO_SUB, txpower, LEN_TX_POWER);
    usleep(TSMS_WAIT); writeBytes(RF_CONF, RF_RXCTRLH_SUB, rfrxctrlh, LEN_RF_RXCTRLH);
    usleep(TSMS_WAIT); writeBytes(RF_CONF, RF_TXCTRL_SUB, rftxctrl, LEN_RF_TXCTRL);
    usleep(TSMS_WAIT); writeBytes(TX_CAL, TC_PGDELAY_SUB, tcpgdelay, LEN_TC_PGDELAY);
    usleep(TSMS_WAIT); writeBytes(FS_CTRL, FS_PLLTUNE_SUB, fsplltune, LEN_FS_PLLTUNE);
    usleep(TSMS_WAIT); writeBytes(FS_CTRL, FS_PLLCFG_SUB, fspllcfg, LEN_FS_PLLCFG);
    usleep(TSMS_WAIT); writeBytes(FS_CTRL, FS_XTALT_SUB, fsxtalt, LEN_FS_XTALT);
}
...
void DW1000Class::handleInterrupt() {
    ...
    while(true) {
        int ret = gpioGetValue(pinIRQ_, &val);
        if (1 != val) {
...
void DW1000Class::getPrintableDeviceIdentifier(char msgBuffer[]) {
    ...
    usleep(TSMS_WAIT); readBytes(DEV_ID, NO_SUB, data, LEN_DEV_ID);
...
void DW1000Class::getPrintableExtendedUniqueIdentifier(char msgBuffer[]) {
    byte data[LEN_EUI];
    usleep(TSMS_WAIT); readBytes(EUI, NO_SUB, data, LEN_EUI);
...
void DW1000Class::getPrintableNetworkIdAndShortAddress(char msgBuffer[]) {
    ...
    usleep(TSMS_WAIT); readBytes(PANADR, NO_SUB, data, LEN_PANADR);
...
void DW1000Class::readSystemConfigurationRegister() {
    usleep(TSMS_WAIT); readBytes(SYS_CFG, NO_SUB, _syscfg, LEN_SYS_CFG);
}

void DW1000Class::writeSystemConfigurationRegister() {
    usleep(TSMS_WAIT); writeBytes(SYS_CFG, NO_SUB, _syscfg, LEN_SYS_CFG);
}

void DW1000Class::readSystemEventStatusRegister() {
    usleep(TSMS_WAIT); readBytes(SYS_STATUS, NO_SUB, _sysstatus, LEN_SYS_STATUS);
}

void DW1000Class::readNetworkIdAndDeviceAddress() {
    usleep(TSMS_WAIT); readBytes(PANADR, NO_SUB, _networkAndAddress, LEN_PANADR);
}

void DW1000Class::writeNetworkIdAndDeviceAddress() {
    usleep(TSMS_WAIT); writeBytes(PANADR, NO_SUB, _networkAndAddress, LEN_PANADR);
}

void DW1000Class::readSystemEventMaskRegister() {
    usleep(TSMS_WAIT); readBytes(SYS_MASK, NO_SUB, _sysmask, LEN_SYS_MASK);
}

void DW1000Class::writeSystemEventMaskRegister() {
    usleep(TSMS_WAIT); writeBytes(SYS_MASK, NO_SUB, _sysmask, LEN_SYS_MASK);
}

void DW1000Class::readChannelControlRegister() {
    usleep(TSMS_WAIT); readBytes(CHAN_CTRL, NO_SUB, _chanctrl, LEN_CHAN_CTRL);
}

void DW1000Class::writeChannelControlRegister() {
    usleep(TSMS_WAIT); writeBytes(CHAN_CTRL, NO_SUB, _chanctrl, LEN_CHAN_CTRL);
}

void DW1000Class::readTransmitFrameControlRegister() {
    usleep(TSMS_WAIT); readBytes(TX_FCTRL, NO_SUB, _txfctrl, LEN_TX_FCTRL);
}

void DW1000Class::writeTransmitFrameControlRegister() {
    usleep(TSMS_WAIT); writeBytes(TX_FCTRL, NO_SUB, _txfctrl, LEN_TX_FCTRL);
}
...
void DW1000Class::setEUI(byte eui[]) {
    ...
    usleep(TSMS_WAIT); writeBytes(EUI, NO_SUB, reverseEUI, LEN_EUI);
}
...
void DW1000Class::idle() {
    ...
    _deviceMode = IDLE_MODE;
    usleep(TSMS_WAIT); writeBytes(SYS_CTRL, NO_SUB, _sysctrl, LEN_SYS_CTRL);
}
...
void DW1000Class::commitConfiguration() {
    ...
    usleep(TSMS_WAIT); writeBytes(TX_ANTD, NO_SUB, antennaDelayBytes, LEN_TX_ANTD);
    usleep(TSMS_WAIT); writeBytes(LDE_IF, LDE_RXANTD_SUB, antennaDelayBytes, LEN_LDE_RXANTD);
}
...
DW1000Time DW1000Class::setDelay(const DW1000Time& delay) {
    ...
    delayBytes[0] = 0;
    delayBytes[1] &= 0xFE;
    usleep(TSMS_WAIT); writeBytes(DX_TIME, NO_SUB, delayBytes, LEN_DX_TIME);
...
void DW1000Class::setDataRate(byte rate) {
    ...
    usleep(TSMS_WAIT); writeBytes(USR_SFD, SFD_LENGTH_SUB, &sfdLength, LEN_SFD_LENGTH);
    _dataRate = rate;
}
...
void DW1000Class::setData(byte data[], uint16_t n) {
    ...
    usleep(TSMS_WAIT); writeBytes(TX_BUFFER, NO_SUB, data, n);
...
void DW1000Class::setData(const String& data) {
    ...
    std::copy(std::begin(data), std::end(data), dataBytes);
    setData(dataBytes, n);
    free(dataBytes);
}
...
uint16_t DW1000Class::getDataLength() {
    ...
    if(_deviceMode == TX_MODE) {
        ...
        usleep(TSMS_WAIT); readBytes(RX_FINFO, NO_SUB, rxFrameInfo, LEN_RX_FINFO);
...
void DW1000Class::getData(byte data[], uint16_t n) {
    ...
    usleep(TSMS_WAIT); readBytes(RX_BUFFER, NO_SUB, data, n);
}
...
void DW1000Class::getTransmitTimestamp(DW1000Time& time) {
    ...
    usleep(TSMS_WAIT); readBytes(TX_TIME, TX_STAMP_SUB, txTimeBytes, LEN_TX_STAMP);
    time.setTimestamp(txTimeBytes);
}

void DW1000Class::getReceiveTimestamp(DW1000Time& time) {
    byte rxTimeBytes[LEN_RX_STAMP];
    usleep(TSMS_WAIT); readBytes(RX_TIME, RX_STAMP_SUB, rxTimeBytes, LEN_RX_STAMP);
...
void DW1000Class::getSystemTimestamp(DW1000Time& time) {
    ...
    usleep(TSMS_WAIT); readBytes(SYS_TIME, NO_SUB, sysTimeBytes, LEN_SYS_TIME);
    time.setTimestamp(sysTimeBytes);
}

void DW1000Class::getTransmitTimestamp(byte data[]) {
    usleep(TSMS_WAIT); readBytes(TX_TIME, TX_STAMP_SUB, data, LEN_TX_STAMP);
}

void DW1000Class::getReceiveTimestamp(byte data[]) {
    usleep(TSMS_WAIT); readBytes(RX_TIME, RX_STAMP_SUB, data, LEN_RX_STAMP);
}

void DW1000Class::getSystemTimestamp(byte data[]) {
    usleep(TSMS_WAIT); readBytes(SYS_TIME, NO_SUB, data, LEN_SYS_TIME);
}
...
void DW1000Class::clearAllStatus() {
    ...
    usleep(TSMS_WAIT); writeBytes(SYS_STATUS, NO_SUB, _sysstatus, LEN_SYS_STATUS);
}
...
float DW1000Class::getReceiveQuality() {
    ...
    uint16_t     noise, f2;
    usleep(TSMS_WAIT); readBytes(RX_FQUAL, STD_NOISE_SUB, noiseBytes, LEN_STD_NOISE);
    usleep(TSMS_WAIT); readBytes(RX_FQUAL, FP_AMPL2_SUB, fpAmpl2Bytes, LEN_FP_AMPL2);
...
float DW1000Class::getFirstPathPower() {
    ...
    uint16_t     f1, f2, f3, N;
    float        A, corrFac;
    usleep(TSMS_WAIT); readBytes(RX_TIME, FP_AMPL1_SUB, fpAmpl1Bytes, LEN_FP_AMPL1);
    usleep(TSMS_WAIT); readBytes(RX_FQUAL, FP_AMPL2_SUB, fpAmpl2Bytes, LEN_FP_AMPL2);
    usleep(TSMS_WAIT); readBytes(RX_FQUAL, FP_AMPL3_SUB, fpAmpl3Bytes, LEN_FP_AMPL3);
    usleep(TSMS_WAIT); readBytes(RX_FINFO, NO_SUB, rxFrameInfo, LEN_RX_FINFO);
...
float DW1000Class::getReceivePower() {
    ...
    uint16_t C, N;
    float         A, corrFac;
    usleep(TSMS_WAIT); readBytes(RX_FQUAL, CIR_PWR_SUB, cirPwrBytes, LEN_CIR_PWR);
    usleep(TSMS_WAIT); readBytes(RX_FINFO, NO_SUB, rxFrameInfo, LEN_RX_FINFO);
...
void DW1000Class::readBytes(byte cmd, uint16_t offset, byte data[], uint16_t n) {
    std::lock_guard<std::mutex> lk(mutexRD_);
    byte header[3];
    uint8_t headerLen = 1;
    if(offset == NO_SUB) {
        header[0] = READ | cmd;
...
void DW1000Class::readBytesOTP(uint16_t address, byte data[]) {
    ...
    // set address
    usleep(TSMS_WAIT); writeBytes(OTP_IF, OTP_ADDR_SUB, addressBytes, LEN_OTP_ADDR);
    otpctrl[0] = 0x03;
    usleep(TSMS_WAIT); writeBytes(OTP_IF, OTP_CTRL_SUB, otpctrl, LEN_OTP_CTRL);
    otpctrl[0] = 0x01;
    usleep(TSMS_WAIT); writeBytes(OTP_IF, OTP_CTRL_SUB, otpctrl, LEN_OTP_CTRL);
    // read value
    usleep(TSMS_WAIT); readBytes(OTP_IF, OTP_RDAT_SUB, data, LEN_OTP_RDAT);
    otpctrl[0] = 0x00;
    usleep(TSMS_WAIT); writeBytes(OTP_IF, OTP_CTRL_SUB, otpctrl, LEN_OTP_CTRL);
}
 ...
 * 		the register).
 */
void DW1000Class::writeBytes(byte cmd, uint16_t offset, byte data[], uint16_t n) {
    std::lock_guard<std::mutex> lk(mutexWR_);
    byte header[3];
    uint8_t  headerLen = 1;
...
void DW1000Class::getPrettyBytes(byte cmd, uint16_t offset, char msgBuffer[], uint16_t n) {
    ...
    usleep(TSMS_WAIT); readBytes(cmd, offset, readBuf, n);
