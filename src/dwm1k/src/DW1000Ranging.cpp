...
timepoint             DW1000RangingClass::timer = std::chrono::system_clock::now();
...
timepoint DW1000RangingClass::_lastActivity;
...
timepoint DW1000RangingClass::_rangingCountPeriod = std::chrono::system_clock::now();
//Here our handlers
...
void DW1000RangingClass::generalStart() {
    ...
    _rangingCountPeriod = std::chrono::system_clock::now();
}
...
void DW1000RangingClass::checkForReset() {
    timepoint curMillis = std::chrono::system_clock::now();
    if(!_sentAck && !_receivedAck) {
        // check if inactive
        if((unsigned long)(std::chrono::duration_cast<std::chrono::milliseconds>(curMillis - _lastActivity).count()) > _resetPeriod) {
            resetInactive();
        }
        return;
    }
...
void DW1000RangingClass::loop() {
    ...
    if((unsigned long)(std::chrono::duration_cast<std::chrono::milliseconds>(time - timer).count()) > _timerDelay) {
        timer = time;
        timerTick();
    }
